...
ISP_SNS_MIRRORFLIP_TYPE_E g_aeOs08a20_MirrorFip[VI_MAX_PIPE_NUM] = {0};
...
#define OS08A20_L2S_ADDR		0x3798
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		...
		CVI_U32 SexpLimit = g_astOs08a20_mode[pstSnsState->u8ImgMode].u32L2S_MAX - 1;
		...
		g_astOs08a20_State[ViPipe].u32Sexp_MAX = u32VMAX -
			g_astOs08a20_mode[pstSnsState->u8ImgMode].u16TopBoundary -
			g_astOs08a20_mode[pstSnsState->u8ImgMode].u16BotBoundary -
			g_astOs08a20_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height - 4;
		if (g_au16Os08a20_L2SMode[ViPipe] == SNS_L2S_MODE_AUTO) {
			/* max sexp = blanking - 2 * (l2s_offset + 1)*/
			g_astOs08a20_State[ViPipe].u32Sexp_MAX = g_astOs08a20_State[ViPipe].u32Sexp_MAX -
				(g_astOs08a20_mode[pstSnsState->u8ImgMode].u16L2sOffset + 1) * 2;
		} else {
			/* max sexp = R3798|R3799 - 1*/
			if (g_astOs08a20_State[ViPipe].u32Sexp_MAX > SexpLimit)
				g_astOs08a20_State[ViPipe].u32Sexp_MAX = SexpLimit;
		}
		...
		pstSnsRegsInfo->astI2cData[WDR2_L2S_0].u32Data =
			(((g_astOs08a20_State[ViPipe].u32Sexp_MAX + 1) & 0xFF00) >> 8);
		pstSnsRegsInfo->astI2cData[WDR2_L2S_1].u32Data =
			((g_astOs08a20_State[ViPipe].u32Sexp_MAX + 1) & 0xFF);
	}
		...
		CVI_U32 *au32IntTimeMax, CVI_U32 *au32IntTimeMin, CVI_U32 *pu32LFMaxIntTime)
{
	...
	u32IntTimeMaxTmp  = (g_astOs08a20_State[ViPipe].u32Sexp_MAX < u32IntTimeMaxTmp) ?
					g_astOs08a20_State[ViPipe].u32Sexp_MAX : u32IntTimeMaxTmp;
	 ...
	 */
	if (g_au16Os08a20_L2SMode[ViPipe] == SNS_L2S_MODE_FIX) {
		u32IntTimeMaxTmp1  = ((pstSnsState->au32FL[0] - 4 -
					g_astOs08a20_State[ViPipe].u32Sexp_MAX - 1) * 0x40) / DIV_0_TO_1(au32Ratio[0]);
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	...
	if (pstSnsState->enWDRMode != WDR_MODE_NONE) {
		...
		if (g_au16Os08a20_L2SMode[ViPipe] == SNS_L2S_MODE_AUTO) {
			 ...
			 */
			dummy = g_astOs08a20_State[ViPipe].u32Sexp_MAX + 2 +
				pstMode->u16TopBoundary + pstMode->u16BotBoundary;
			max_dummy = dummy;
		}
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	case WDR_MODE_NONE:
		...
		else if (pstSnsState->u8ImgMode == OS08A20_MODE_3840X2160P30_WDR)
			pstSnsState->u8ImgMode = OS08A20_MODE_3840X2160P30;
		else if (pstSnsState->u8ImgMode == OS08A20_MODE_2560X1440P30_WDR)
			pstSnsState->u8ImgMode = OS08A20_MODE_2560X1440P30;
		else {
		}
	...
	case WDR_MODE_2To1_LINE:
		...
		} else if (pstSnsState->u8ImgMode == OS08A20_MODE_3840X2160P30) {
			pstSnsState->u8ImgMode = OS08A20_MODE_3840X2160P30_WDR;
			syslog(LOG_INFO, "WDR_MODE_2To1_LINE 2160p mode(60fps->30fps)\n");
		} else if (pstSnsState->u8ImgMode == OS08A20_MODE_2560X1440P30) {
			pstSnsState->u8ImgMode = OS08A20_MODE_2560X1440P30_WDR;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		case WDR_MODE_2To1_LINE:
			...
			pstI2c_data[WDR2_L2S_0].u32RegAddr = OS08A20_L2S_ADDR;
			pstI2c_data[WDR2_L2S_1].u32RegAddr = OS08A20_L2S_ADDR + 1;
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (OS08A20_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_2560X1440P30;
			else if (OS08A20_RES_IS_1944P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_2592X1944P30;
			else if (OS08A20_RES_IS_2160P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_3840X2160P30;
			else {
			...
			if (OS08A20_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_2560X1440P30_WDR;
			else if (OS08A20_RES_IS_1944P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_2592X1944P30_WDR;
			else if (OS08A20_RES_IS_2160P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
				u8SensorImageMode = OS08A20_MODE_3840X2160P30_WDR;
			else {
				CVI_TRACE_SNS(CVI_DBG_ERR, "Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
				       pstSensorImageMode->u16Width,
				       pstSensorImageMode->u16Height,
...
static CVI_VOID sensor_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	...
	if (pstSnsState->bInit == CVI_TRUE && g_aeOs08a20_MirrorFip[ViPipe] != eSnsMirrorFlip) {
		os08a20_mirror_flip(ViPipe, eSnsMirrorFlip);
		g_aeOs08a20_MirrorFip[ViPipe] = eSnsMirrorFlip;
	}
}

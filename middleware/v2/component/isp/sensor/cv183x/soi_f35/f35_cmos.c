...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode) {
		g_astF35_State[ViPipe].u32Sexp_MAX = u32VMAX -
			g_astF35_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height * 2;
		/* Sexp = 1,3,5,7... */
		if (g_astF35_State[ViPipe].u32Sexp_MAX < 2) {
			g_astF35_State[ViPipe].u32Sexp_MAX = 1;
		} else {
			/* maximum sexp = 2 * reg_AA + 1 - 8. */
			CVI_U32 SexpLimit = g_astF35_mode[pstSnsState->u8ImgMode].u32L2S_MAX - 4;

			SexpLimit = SexpLimit * 2 + 1;
			g_astF35_State[ViPipe].u32Sexp_MAX = (g_astF35_State[ViPipe].u32Sexp_MAX & (~0x1)) - 1;
			g_astF35_State[ViPipe].u32Sexp_MAX = (g_astF35_State[ViPipe].u32Sexp_MAX > SexpLimit) ?
				SexpLimit : g_astF35_State[ViPipe].u32Sexp_MAX;
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		...
		pstSnsRegsInfo->astI2cData[WDR2_L2S_DATA].u32Data =
			((g_astF35_mode[pstSnsState->u8ImgMode].u32L2S_MAX) & 0xFF);
	}
...
static CVI_S32 cmos_get_cif_wdr(VI_PIPE ViPipe, ISP_SNS_CIF_INFO_S *pstCifCfg)
{
	...
	pstCifCfg->wdr_manual.devno = f35_rx_attr.devno;
	pstCifCfg->wdr_manual.manual_en = 1;
	if (g_au16F35_L2SMode[ViPipe] == SNS_L2S_MODE_AUTO)
		pstCifCfg->wdr_manual.l2s_distance = g_astF35_State[ViPipe].u8SexpReg;
	else
		pstCifCfg->wdr_manual.l2s_distance = (((g_astF35_State[ViPipe].u32Sexp_MAX - 1) >> 1) & 0xFF);
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		case WDR_MODE_2To1_LINE:
			...
			pstI2c_data[WDR2_L2S_DATA].u32RegAddr = F35_L2S_ADDR;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	if (pstSnsState->enWDRMode != WDR_MODE_NONE) {
		...
		if (g_au16F35_L2SMode[ViPipe] == SNS_L2S_MODE_AUTO)
			pstRxAttr->wdr_manu.l2s_distance = g_astF35_State[ViPipe].u8SexpReg;
		else

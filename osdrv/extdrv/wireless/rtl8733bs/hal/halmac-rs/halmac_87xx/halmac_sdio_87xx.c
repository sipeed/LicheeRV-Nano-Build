...
#if (HALMAC_87XX_SUPPORT && HALMAC_SDIO_SUPPORT)
...
/*only for r_indir_sdio_87xx !!, Soar 20171222*/
static u8
r_indir_cmd52_87xx(struct halmac_adapter *adapter, u32 offset);

/*only for r_indir_sdio_87xx !!, Soar 20171222*/
static u32
r_indir_cmd53_87xx(struct halmac_adapter *adapter, u32 offset);

/*only for r_indir_sdio_87xx !!, Soar 20171222*/
static u32
r8_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr);

/*only for r_indir_sdio_87xx !!, Soar 20171222*/
static u32
r16_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr);

/*only for r_indir_sdio_87xx !!, Soar 20171222*/
static u32
r32_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr);

/*only for w_indir_sdio_87xx !!, Soar 20171222*/
static enum halmac_ret_status
w_indir_cmd52_87xx(struct halmac_adapter *adapter, u32 adr, u32 val,
		   enum halmac_io_size size);

/*only for w_indir_sdio_87xx !!, Soar 20171222*/
static enum halmac_ret_status
w_indir_cmd53_87xx(struct halmac_adapter *adapter, u32 adr, u32 val,
		   enum halmac_io_size size);

/*only for w_indir_sdio_87xx !!, Soar 20171222*/
static enum halmac_ret_status
w8_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val);

/*only for w_indir_sdio_87xx !!, Soar 20171222*/
static enum halmac_ret_status
w16_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val);

/*only for w_indir_sdio_87xx !!, Soar 20171222*/
static enum halmac_ret_status
w32_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val);

/**
 * init_sdio_cfg_87xx() - init SDIO
...
init_sdio_cfg_87xx(struct halmac_adapter *adapter)
{
	return HALMAC_RET_SUCCESS;
}

/**
 * deinit_sdio_cfg_87xx() - deinit SDIO
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
deinit_sdio_cfg_87xx(struct halmac_adapter *adapter)
{
...
/**
 * cfg_sdio_rx_agg_87xx() - config rx aggregation
 * @adapter : the adapter of halmac
 * @halmac_rx_agg_mode
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 ...
 */
enum halmac_ret_status
cfg_sdio_rx_agg_87xx(struct halmac_adapter *adapter,
		     struct halmac_rxagg_cfg *cfg)
{
	...
	rxdma_store= HALMAC_REG_R32(REG_RXDMA_AGG_PG_TH);
	...
	//case HALMAC_RX_AGG_MODE_USB:  sdio do not have USB mode  
		agg_enable |= BIT_RXDMA_AGG_EN;
		rxdma_store |= BIT_RXDMA_STORE;
		break;
	default:
	...
	HALMAC_REG_W32(REG_RXDMA_AGG_PG_TH,rxdma_store);//sdio dma mode must be set to 1 jerry_zhou
...
/**
 * sdio_reg_rn_87xx() - read n byte register
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
sdio_reg_rn_87xx(struct halmac_adapter *adapter, u32 offset, u32 size,
		 u8 *value)
{
	u8 *r_val = NULL;
	u32 r_size = size;
	...
	status = cnv_to_sdio_bus_offset_87xx(adapter, &offset);
...
/**
 * cfg_txagg_sdio_align_87xx() -config sdio bus tx agg alignment
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
cfg_txagg_sdio_align_87xx(struct halmac_adapter *adapter, u8 enable,
			  u16 align_size)
{
	u8 i;
	u8 flag = 0;
	...
	if (enable)
		HALMAC_REG_W16(REG_TX_ALIGNMENT_CTRL, 0x8000 | align_size);  
	else
		HALMAC_REG_W16(REG_TX_ALIGNMENT_CTRL, align_size);
...
/**
 * sdio_indirect_reg_r32_87xx() - read MAC reg by SDIO reg
 * @adapter : the adapter of halmac
 * @offset : register offset
 * Author : Soar
 * Return : enum halmac_ret_status
 ...
 */
u32
sdio_indirect_reg_r32_87xx(struct halmac_adapter *adapter, u32 offset)
{
	return r_indir_sdio_87xx(adapter, offset, HALMAC_IO_DWORD);
}

/**
 * set_sdio_bulkout_num_87xx() - inform bulk-out num
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
set_sdio_bulkout_num_87xx(struct halmac_adapter *adapter, u8 num)
{
	return HALMAC_RET_NOT_SUPPORT;
}

/**
 * get_sdio_bulkout_id_87xx() - get bulk out id for the TX packet
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
get_sdio_bulkout_id_87xx(struct halmac_adapter *adapter, u8 *buf, u32 size,
			 u8 *id)
{
	return HALMAC_RET_NOT_SUPPORT;
}
...
/**
 * sdio_cmd53_4byte_87xx() - cmd53 only for 4byte len register IO
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
sdio_cmd53_4byte_87xx(struct halmac_adapter *adapter,
...
/**
 * sdio_hw_info_87xx() - info sdio hw info
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
sdio_hw_info_87xx(struct halmac_adapter *adapter,
...
void
cfg_sdio_tx_page_threshold_87xx(struct halmac_adapter *adapter,
				struct halmac_tx_page_threshold_info *info)
{
	...
	*/
	...
	case HALMAC_MAP2_HQ:
		HALMAC_REG_W16(REG_TQPNT1, (u16)threshold);
		break;
	case HALMAC_MAP2_NQ:
		HALMAC_REG_W16(REG_TQPNT1+2, (u16)threshold);
		break;
	case HALMAC_MAP2_LQ:
		HALMAC_REG_W16(REG_TQPNT2, (u16)threshold);
		break;
	case HALMAC_MAP2_EXQ:
		HALMAC_REG_W16(REG_TQPNT2+2, (u16)threshold);
		break;
	default:
		break;
	}
...
enum halmac_ret_status
cnv_to_sdio_bus_offset_87xx(struct halmac_adapter *adapter, u32 *offset)
{
...
enum halmac_ret_status
leave_sdio_suspend_87xx(struct halmac_adapter *adapter)
{
	u8 value8;
	u32 cnt;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
r_indir_cmd52_87xx(struct halmac_adapter *adapter, u32 offset)
{
	...
	status = cnv_to_sdio_bus_offset_87xx(adapter, &reg_cfg);
	if (status != HALMAC_RET_SUCCESS)
		return status;
	status = cnv_to_sdio_bus_offset_87xx(adapter, &reg_data);
	if (status != HALMAC_RET_SUCCESS)
		return status;
/*
...
*/
	...
	/*jerry_zhou
	to avoid HCI read page0,and the same time cpu write page0, HCI will change read to write;
	set 0x10250040[17:16]=2'b11
	0x10250040[15:0]: indirect reg address
	0x10250040[17:16]: indirect reg size
	0x10250040[18]: indirect reg write,for write operation,set 1
	0x10250040[19]: indirect reg read,for read operation,set 1
	0x10250040[20]: indirect reg rdy, polling to 1, indicates the indirect access R/W is ready
	*/
	PLTFM_SDIO_CMD52_W(reg_cfg, (u8)offset);
	PLTFM_SDIO_CMD52_W(reg_cfg + 1, (u8)(offset >> 8));
	rdata=PLTFM_SDIO_CMD52_R(reg_cfg+2);
	PLTFM_SDIO_CMD52_W(reg_cfg + 2, (u8)(BIT(0) | BIT(1)));
	PLTFM_SDIO_CMD52_W(reg_cfg + 2, (u8)(BIT(0) | BIT(1)| BIT(3)| BIT(4)));
	...
	temp= PLTFM_SDIO_CMD52_R(reg_cfg + 2);
	temp=(temp & 0xFC)| (rdata & 0x03);
	PLTFM_SDIO_CMD52_W(reg_cfg + 2,temp);//recover value 0x10250042[1:0]
...
r_indir_cmd53_87xx(struct halmac_adapter *adapter, u32 offset)
{
	...
	0x10250040[20]: indirect reg rdy, polling to 1, indicates the indirect access R/W is ready
	*/
	rdata= PLTFM_SDIO_CMD53_R32(reg_cfg);
	temp= rdata| BIT(16)| BIT(17);
	PLTFM_SDIO_CMD53_W32(reg_cfg, temp);
	temp= (temp & 0xFFFF0000)| offset | BIT(19)| BIT(20);
	PLTFM_SDIO_CMD53_W32(reg_cfg, temp);
	...
	temp=PLTFM_SDIO_CMD53_R32(reg_cfg);
	temp=(temp & 0xFFFCFFFF) | (rdata & 0x00030000); //recover value 0x10250040[17:16]
	PLTFM_SDIO_CMD53_W32(reg_cfg,temp);
...
r8_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr)
{
	...
	if (adapter->pwr_off_flow_flag == 1 ||
	    adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		val.byte[0] = r_indir_cmd52_87xx(adapter, adr);
		return rtk_le32_to_cpu(val.dword);
	}

	return r_indir_cmd53_87xx(adapter, adr);
}
...
r16_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr)
{
	...
	if (adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		if (0 != (adr & (2 - 1))) {
			val.byte[0] = r_indir_cmd52_87xx(adapter, adr);
			val.byte[1] = r_indir_cmd52_87xx(adapter, adr + 1);
		} else {
			val.byte[0] = r_indir_cmd52_87xx(adapter, adr);
	...
	if (0 != (adr & (2 - 1))) {
		val.byte[0] = (u8)r_indir_cmd53_87xx(adapter, adr);
		val.byte[1] = (u8)r_indir_cmd53_87xx(adapter, adr + 1);
...
r32_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr)
{
	...
	if (adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		if (0 != (adr & (4 - 1))) {
			val.byte[0] = r_indir_cmd52_87xx(adapter, adr);
			val.byte[1] = r_indir_cmd52_87xx(adapter, adr + 1);
			val.byte[2] = r_indir_cmd52_87xx(adapter, adr + 2);
			val.byte[3] = r_indir_cmd52_87xx(adapter, adr + 3);
		} else {
			val.byte[0] = r_indir_cmd52_87xx(adapter, adr);
			val.byte[1] = PLTFM_SDIO_CMD52_R(reg_data + 1);
	...
	if (0 != (adr & (4 - 1))) {
		val.byte[0] = (u8)r_indir_cmd53_87xx(adapter, adr);
		val.byte[1] = (u8)r_indir_cmd53_87xx(adapter, adr + 1);
		val.byte[2] = (u8)r_indir_cmd53_87xx(adapter, adr + 2);
		val.byte[3] = (u8)r_indir_cmd53_87xx(adapter, adr + 3);
...
u32
r_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr,
		  enum halmac_io_size size)
{
	u32 value32 = 0;
	...
	switch (size) {
	case HALMAC_IO_BYTE:
		value32 = r8_indir_sdio_87xx(adapter, adr);
		break;
	case HALMAC_IO_WORD:
		value32 = r16_indir_sdio_87xx(adapter, adr);
		break;
	case HALMAC_IO_DWORD:
		value32 = r32_indir_sdio_87xx(adapter, adr);
		break;
	default:
		break;
	}
...
w8_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val)
{
	...
	if (adapter->pwr_off_flow_flag == 1 ||
	    adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF)
		status = w_indir_cmd52_87xx(adapter, adr, val, HALMAC_IO_BYTE);
	else
		status = w_indir_cmd53_87xx(adapter, adr, val, HALMAC_IO_BYTE);
	return status;
}
...
w16_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val)
{
	...
	if (adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		if (0 != (adr & (2 - 1))) {
			status = w_indir_cmd52_87xx(adapter, adr, val,
						    HALMAC_IO_BYTE);
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd52_87xx(adapter, adr + 1, val >> 8,
						    HALMAC_IO_BYTE);
		} else {
			status = w_indir_cmd52_87xx(adapter, adr, val,
						    HALMAC_IO_WORD);
		...
		if (0 != (adr & (2 - 1))) {
			status = w_indir_cmd53_87xx(adapter, adr, val,
						    HALMAC_IO_BYTE);
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd53_87xx(adapter, adr + 1, val >> 8,
						    HALMAC_IO_BYTE);
		} else {
			status = w_indir_cmd53_87xx(adapter, adr, val,
						    HALMAC_IO_WORD);
...
w32_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val)
{
	...
	if (adapter->halmac_state.mac_pwr == HALMAC_MAC_POWER_OFF) {
		if (0 != (adr & (4 - 1))) {
			...
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd52_87xx(adapter, adr + 2, val >> 16,
						    HALMAC_IO_BYTE);
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd52_87xx(adapter, adr + 3, val >> 24,
						    HALMAC_IO_BYTE);
		} else {
			status = w_indir_cmd52_87xx(adapter, adr, val,
						    HALMAC_IO_DWORD);
		...
		if (0 != (adr & (4 - 1))) {
			...
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd53_87xx(adapter, adr + 2, val >> 16,
						    HALMAC_IO_BYTE);
			if (status != HALMAC_RET_SUCCESS)
				return status;
			status = w_indir_cmd53_87xx(adapter, adr + 3, val >> 24,
						    HALMAC_IO_BYTE);
		} else {
			status = w_indir_cmd53_87xx(adapter, adr, val,
						    HALMAC_IO_DWORD);
...
enum halmac_ret_status
w_indir_sdio_87xx(struct halmac_adapter *adapter, u32 adr, u32 val,
		  enum halmac_io_size size)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	...
	switch (size) {
	case HALMAC_IO_BYTE:
		status = w8_indir_sdio_87xx(adapter, adr, val);
		break;
	case HALMAC_IO_WORD:
		status = w16_indir_sdio_87xx(adapter, adr, val);
		break;
	case HALMAC_IO_DWORD:
		status = w32_indir_sdio_87xx(adapter, adr, val);
		break;
	default:
		break;
	}
...
enum halmac_ret_status
en_ref_autok_sdio_87xx(struct halmac_adapter *adapter, u8 en)
{
	return HALMAC_RET_NOT_SUPPORT;
}

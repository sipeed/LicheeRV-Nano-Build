...
static void conv_image_not_pack_output_planar(
	unsigned short *outbuf, unsigned short *inbuf,
	int pic_width, int pic_height, int out_width, int out_height)
{
	int x = 0, y = 0;
	int iaddr = 0, oaddr = 0;
	int temp = 0, tbase = 0;
	...
	// V
	tbase = (pic_width * pic_height * 5) >> 2;
	iaddr = tbase;
	oaddr = (out_width * out_height * 5) >> 2;
	for (y = 0; y < (out_height >> 1); y++) {
		for (x = 0; x < (out_width >> 1); x++) {
			if (y >= (pic_height >> 1)) {
				if (x >= (pic_width >> 1))
...
static void conv_image_not_pack_output_semiplanar(
	unsigned short *outbuf, unsigned short *inbuf,
	int pic_width, int pic_height, int out_width, int out_height, int frm_format)
{
	int x = 0, y = 0;
	int iaddr = 0, oaddr = 0;
	int temp = 0, tbase = 0;
	...
	// U
	tbase = (frm_format == OUTPUT_NV12) ?
			      pic_width * pic_height :
			      ((pic_width * pic_height * 5) >> 2);
	iaddr = tbase;
	oaddr = out_width * out_height;
	for (y = 0; y < (out_height >> 1); y++) {
		for (x = 0; x < (out_width >> 1); x++) {
...
void conv_image(unsigned short *outbuf, unsigned short *inbuf, int pic_width,
		int pic_height, int out_width, int out_height, int frm_format,
		int packedFormat)
{
	int x = 0, y = 0;
	int iaddr = 0, oaddr = 0;
	...
	if (packedFormat == NOT_PACKED) {
		if (frm_format == OUTPUT_PLANAR) { // planar
			conv_image_not_pack_output_planar(outbuf, inbuf,
				pic_width, pic_height, out_width, out_height);
		} else if (frm_format == OUTPUT_NV12 ||
			   frm_format == OUTPUT_NV21) { // NV12, NV21
			conv_image_not_pack_output_semiplanar(outbuf, inbuf,
				pic_width, pic_height, out_width, out_height, frm_format)
		} else {
			printf("Error) Unknown frame format: %d\n", frm_format);
		}
	} else { /* packdformat */
		if (packedFormat == PACKED_YUYV ||
		    packedFormat == PACKED_YVYU) {
			oaddr = 0;
		} else {
			oaddr = 1;
		}
...
int write_image_to_file(FILE *fp, unsigned short *outbuf, int out_width,
			int out_height, int pxl_format, int frm_format,
			int packedFormat)
{
	int i = 0, x = 0, y = 0;
	...
	size = (packedFormat) ? out_width * out_height * 2 :
				      ((out_width * out_height * 3) >> 1);
	//***** 8bit
	if (pxl_format == 0) {
		for (i = 0; i < size; i++)
	...
	//***** 32bit
		...
		width = (frm_format == 2 || frm_format == 3) ? out_width :
								     (out_width >> 1);
		height = out_height >> 1;
		// Cb
		for (y = 0; y < height; y++) {
...
int write_image_to_mem(unsigned short *out_mem, unsigned short *inbuf,
		       int out_width, int out_height, int pxl_format,
		       int frm_format, int packedFormat)
{
	int i, x, y;
	unsigned int tmp32 = 0;
	int width = 0, height = 0;
			  ...
			  size_t picWidth, size_t picHeight, void *arg)
{
...

	{
		...
		if (yuv3p4b == 1) // calculate input YUV(16BIT) size
			CalcYuvSize(FORMAT_420_P10_16BIT_LSB, picWidth,
				    picHeight, fb->cbcrInterleave, &lumaSize,
				    &chromaSize, &srcFrameSize, NULL, NULL,
				    NULL);
		else if (bitdepth == 10)
			CalcYuvSize(FORMAT_420_P10_16BIT_LSB, picWidth,
				    picHeight, fb->cbcrInterleave, &lumaSize,
			...
			CalcYuvSize(input_format, picWidth, picHeight,
				    fb->cbcrInterleave, &lumaSize, &chromaSize,
				    &srcFrameSize, NULL, NULL, NULL);
	}

	out_width = (yuv3p4b && ctx->packedFormat == 0) ?
				  (((picWidth + 31) >> 5) << 5) :
				  picWidth;
	out_height = (yuv3p4b) ? (((picHeight + 7) >> 3) << 3) : picHeight;

	outsize = (ctx->packedFormat) ? out_width * out_height * 2 :
					      ((out_width * out_height * 3) >> 1);

	outsize *= 2; // align margin

	pYuv_int16 = (Uint16 *)osal_malloc(outsize * sizeof(Uint16));
	if (pYuv_int16 == NULL) {
		printf("malloc error: outbuf\n");
		return 0;
	}

	pYuv2_int16 = (Uint16 *)osal_malloc(outsize * sizeof(Uint16));
	if (pYuv2_int16 == NULL) {
		printf("malloc error: outbuf\n");
		return 0;
	}
	...
	if (bitdepth == 0) {
		...
		if (yuv3p4b == 0) {
			/**************************************************************
			 * when 10bit source YUV == LITTLE ENDIAN, 16LSB, alway
			 *do byte-swapping
			 **************************************************************/
			for (i = 0; i < (srcFrameSize >> 1); i++) {
				pYuv_int8[i * 2] =
					(Uint8)(pYuv_int16[i] >> 8) & 0xff;
				pYuv_int8[i * 2 + 1] =
					(Uint8)(pYuv_int16[i]) & 0xff;
			}
#if 0
			/***************************************************************
	...
	if (out_frm_format != OUTPUT_PLANAR || ctx->packedFormat || yuv3p4b) {
		conv_image((unsigned short *)pYuv2_int16,
			   ((bitdepth == 0) ? ((unsigned short *)pYuv_int16) :
						    ((unsigned short *)pYuv_int8)),
			   picWidth, picHeight, out_width, out_height,
			   out_frm_format, ctx->packedFormat);
		write_image_to_mem((unsigned short *)pYuv_int8,
				   (unsigned short *)pYuv2_int16, ystride,
				   out_height, pxl_format, out_frm_format,
				   ctx->packedFormat);
	}
...
#if 0
	{
		...
		sprintf(fileName, "loaded_%s_%dx%d_%s.yuv", pixel_string,
			picWidth, picHeight, frm_string);
...
#endif
		if (yuv3p4b == 0)
			write_image_to_file(save_fp,
					    (unsigned short *)pYuv_int8,
					    out_width, out_height, pxl_format,
					    out_frm_format, ctx->packedFormat);
		else
			write_image_to_file(save_fp,
					    (unsigned short *)pYuv2_int16,
					    out_width, out_height, pxl_format,
					    out_frm_format, ctx->packedFormat);
#ifdef SAVE_WHOLE_LOADED_YUV
	...
	if (yuv3p4b && ctx->packedFormat) {
		...
		out_width *= 2; // 8bit packed mode(YUYV) only. (need to add
			// 10bit cases later)
		if (bitdepth != 0) // 10bit packed
			out_width *= 2;
	}
	LoadYuvImageBurstFormat(coreIdx, pYuv_int8, out_width, out_height, fb,
				TRUE);

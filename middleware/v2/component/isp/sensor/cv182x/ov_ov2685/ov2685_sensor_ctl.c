...
#define OV2685_FLIP	0x3820
#define OV2685_MIRROR	0x3821
void ov2685_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	CVI_U8 flip, mirror;

	flip = ov2685_read_register(ViPipe, OV2685_FLIP);
	mirror = ov2685_read_register(ViPipe, OV2685_MIRROR);
	flip &= ~(0x3 << 1);
	mirror &= ~(0x3 << 1);
	...
	ov2685_write_register(ViPipe, OV2685_FLIP, flip);
	ov2685_write_register(ViPipe, OV2685_MIRROR, mirror);
}
...
static void ov2685_linear_1600x1200_30_init(VI_PIPE ViPipe)
{
	...
	ov2685_write_register(ViPipe, 0x3503, 0x0007);

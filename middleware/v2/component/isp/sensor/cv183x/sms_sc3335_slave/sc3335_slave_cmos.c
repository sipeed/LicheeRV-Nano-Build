...
#include "sc3335_slave_cmos_param.h"
#include <linux/cvi_vip_snsr.h>

#define DIV_0_TO_1(a)   ((0 == (a)) ? 1 : (a))
#define DIV_0_TO_1_FLOAT(a) ((((a) < 1E-10) && ((a) > -1E-10)) ? 1 : (a))
#define SC3335_SLAVE_ID 3335
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastSC3335_SLAVE[VI_MAX_PIPE_NUM] = {CVI_NULL};

#define SC3335_SLAVE_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastSC3335_SLAVE[dev])
#define SC3335_SLAVE_SENSOR_SET_CTX(dev, pstCtx)   (g_pastSC3335_SLAVE[dev] = pstCtx)
#define SC3335_SLAVE_SENSOR_RESET_CTX(dev)         (g_pastSC3335_SLAVE[dev] = CVI_NULL)

ISP_SNS_COMMBUS_U g_aunSC3335_SLAVE_BusInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cDev = 2},
	[1 ... VI_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1}
};

CVI_U16 g_au16SC3335_SLAVE_GainMode[VI_MAX_PIPE_NUM] = {0};
CVI_U16 g_au16SC3335_SLAVE_L2SMode[VI_MAX_PIPE_NUM] = {0};

ISP_SNS_MIRRORFLIP_TYPE_E g_aeSc3335_Slave_MirrorFip[VI_MAX_PIPE_NUM] = {0};
...
/*****SC3335_SLAVE Lines Range*****/
#define SC3335_SLAVE_FULL_LINES_MAX  (0x7FFF)

/*****SC3335_SLAVE Register Address*****/
#define SC3335_SLAVE_EXP_H_ADDR                     (0x3e00)
#define SC3335_SLAVE_EXP_M_ADDR                     (0x3e01)
#define SC3335_SLAVE_EXP_L_ADDR                     (0x3e02)

#define SC3335_SLAVE_AGAIN_H_ADDR                   (0x3e08)
#define SC3335_SLAVE_AGAIN_L_ADDR                   (0x3e09)

#define SC3335_SLAVE_DGAIN_H_ADDR                   (0x3e06)
#define SC3335_SLAVE_DGAIN_L_ADDR                   (0x3e07)

#define SC3335_SLAVE_VMAX_H_ADDR                    (0x320e)
#define SC3335_SLAVE_VMAX_L_ADDR                    (0x320f)

#define SC3335_SLAVE_GAIN_LOGIC_H_ADDR              (0x363c)
#define SC3335_SLAVE_GAIN_LOGIC_L_ADDR              (0x330e)

#define SC3335_SLAVE_GAIN_DPC_ADDR                  (0x5799)
#define SC3335_SLAVE_HOLD_ADDR                      (0x3812)

#define SC3335_SLAVE_RES_IS_1296P(w, h)      ((w) <= 2304 && (h) <= 1296)

#define SC3335_SLAVE_EXPACCURACY                    (0.5)

static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	const SC3335_SLAVE_MODE_S *pstMode;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;

	CMOS_CHECK_POINTER(pstAeSnsDft);
	SC3335_SLAVE_SENSOR_GET_CTX(ViPipe, pstSnsState);
	CMOS_CHECK_POINTER(pstSnsState);

	pstMode = &g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode];
#if 0
	memset(&pstAeSnsDft->stAERouteAttr, 0, sizeof(ISP_AE_ROUTE_S));
#endif
	pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
	pstAeSnsDft->u32FlickerFreq = 50 * 256;
	pstAeSnsDft->u32FullLinesMax = SC3335_SLAVE_FULL_LINES_MAX;
	pstAeSnsDft->u32HmaxTimes = (1000000) / (pstSnsState->u32FLStd * 30);

	pstAeSnsDft->stIntTimeAccu.enAccuType = AE_ACCURACY_LINEAR;
	pstAeSnsDft->stIntTimeAccu.f32Accuracy = SC3335_SLAVE_EXPACCURACY;
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32Vts = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
	pstSnsRegsInfo = &pstSnsState->astSyncInfo[0].snsCfg;
	f32MaxFps = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].f32MaxFps;
	f32MinFps = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].f32MinFps;

	switch (pstSnsState->u8ImgMode) {
	case SC3335_SLAVE_MODE_2304X1296P30:
		...
		u32VMAX = (u32VMAX > SC3335_SLAVE_FULL_LINES_MAX) ? SC3335_SLAVE_FULL_LINES_MAX : u32VMAX;
		break;
	default:
...
static CVI_S32 cmos_gains_update(VI_PIPE ViPipe, CVI_U32 *pu32Again, CVI_U32 *pu32Dgain)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		...
		/* gain logic setting. */
		if (sc3335_slave_read_register(ViPipe, 0x3109) == 1) {
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	const SC3335_SLAVE_MODE_S *pstMode = CVI_NULL;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		pstSnsState->u8ImgMode = SC3335_SLAVE_MODE_2304X1296P30;
		pstSnsState->enWDRMode = WDR_MODE_NONE;
		pstSnsState->u32FLStd = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].u32VtsDef;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		pstCfg0->snsCfg.unComBus.s8I2cDev = g_aunSC3335_SLAVE_BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = sc3335_slave_i2c_addr;
			pstI2c_data[i].u32AddrByteNum = sc3335_slave_addr_byte;
			pstI2c_data[i].u32DataByteNum = sc3335_slave_data_byte;
		}

		switch (pstSnsState->enWDRMode) {
		case WDR_MODE_NONE:
			//Linear Mode Regs
			pstI2c_data[LINEAR_EXP_H_ADDR].u32RegAddr = SC3335_SLAVE_EXP_H_ADDR;
			pstI2c_data[LINEAR_EXP_M_ADDR].u32RegAddr = SC3335_SLAVE_EXP_M_ADDR;
			pstI2c_data[LINEAR_EXP_L_ADDR].u32RegAddr = SC3335_SLAVE_EXP_L_ADDR;
			pstI2c_data[LINEAR_AGAIN_H_ADDR].u32RegAddr = SC3335_SLAVE_AGAIN_H_ADDR;
			pstI2c_data[LINEAR_AGAIN_L_ADDR].u32RegAddr = SC3335_SLAVE_AGAIN_L_ADDR;
			pstI2c_data[LINEAR_DGAIN_H_ADDR].u32RegAddr = SC3335_SLAVE_DGAIN_H_ADDR;
			pstI2c_data[LINEAR_DGAIN_L_ADDR].u32RegAddr = SC3335_SLAVE_DGAIN_L_ADDR;
			pstI2c_data[LINEAR_VMAX_H_ADDR].u32RegAddr = SC3335_SLAVE_VMAX_H_ADDR;
			pstI2c_data[LINEAR_VMAX_L_ADDR].u32RegAddr = SC3335_SLAVE_VMAX_L_ADDR;
			pstI2c_data[LINEAR_GAIN_DPC_ADDR].bUpdate = CVI_FALSE;
			pstI2c_data[LINEAR_GAIN_DPC_ADDR].u32RegAddr = SC3335_SLAVE_GAIN_DPC_ADDR;
			pstI2c_data[LINEAR_HOLD].u32RegAddr = SC3335_SLAVE_HOLD_ADDR;
			pstI2c_data[LINEAR_GAIN_LOGIC_H_ADDR].bUpdate = CVI_FALSE;
			pstI2c_data[LINEAR_GAIN_LOGIC_H_ADDR].u32RegAddr = SC3335_SLAVE_GAIN_LOGIC_H_ADDR;
			pstI2c_data[LINEAR_GAIN_LOGIC_L_ADDR].bUpdate = CVI_FALSE;
			pstI2c_data[LINEAR_GAIN_LOGIC_L_ADDR].u32RegAddr = SC3335_SLAVE_GAIN_LOGIC_L_ADDR;
			pstI2c_data[LINEAR_REL].u32RegAddr = SC3335_SLAVE_HOLD_ADDR;
			break;
		default:
			CVI_TRACE_SNS(CVI_DBG_ERR, "Not support WDR: %d\n", pstSnsState->enWDRMode);
			return CVI_FAILURE;
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (SC3335_SLAVE_RES_IS_1296P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = SC3335_SLAVE_MODE_2304X1296P30;
			} else {
...
static CVI_VOID sensor_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
	...
	if (pstSnsState->bInit == CVI_TRUE && g_aeSc3335_Slave_MirrorFip[ViPipe] != eSnsMirrorFlip) {
		sc3335_slave_mirror_flip(ViPipe, eSnsMirrorFlip);
		g_aeSc3335_Slave_MirrorFip[ViPipe] = eSnsMirrorFlip;
	}
}
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	memcpy(pstRxAttr, &sc3335_slave_rx_attr, sizeof(*pstRxAttr));

	pstRxAttr->img_size.width = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Width;
	pstRxAttr->img_size.height = g_astSC3335_SLAVE_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height;
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &sc3335_slave_rx_attr;
	int i;
...
static CVI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_init = sc3335_slave_init;
	pstSensorExpFunc->pfn_cmos_sensor_exit = sc3335_slave_exit;
...
static CVI_S32 sc3335_slave_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
	g_aunSC3335_SLAVE_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static CVI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
	...
	SC3335_SLAVE_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
	...
	SC3335_SLAVE_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static CVI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
	...
	SC3335_SLAVE_SENSOR_RESET_CTX(ViPipe);
}
...
static CVI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	stSnsAttrInfo.eSensorId = SC3335_SLAVE_ID;
...
static CVI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	s32Ret = CVI_ISP_SensorUnRegCallBack(ViPipe, SC3335_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC3335_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC3335_SLAVE_ID);
	if (s32Ret != CVI_SUCCESS) {
...
static CVI_S32 sensor_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
	...
	g_au16SC3335_SLAVE_GainMode[ViPipe] = pstInitAttr->enGainMode;
	g_au16SC3335_SLAVE_L2SMode[ViPipe] = pstInitAttr->enL2SMode;

	return CVI_SUCCESS;
}

ISP_SNS_OBJ_S stSnsSC3335_Slave_Obj = {
	.pfnRegisterCallback    = sensor_register_callback,
	.pfnUnRegisterCallback  = sensor_unregister_callback,
	.pfnStandby             = sc3335_slave_standby,
	.pfnRestart             = sc3335_slave_restart,
	.pfnMirrorFlip          = sensor_mirror_flip,
	.pfnWriteReg            = sc3335_slave_write_register,
	.pfnReadReg             = sc3335_slave_read_register,
	.pfnSetBusInfo          = sc3335_slave_set_bus_info,
	...
	.pfnSnsProbe		= sc3335_slave_probe,
};

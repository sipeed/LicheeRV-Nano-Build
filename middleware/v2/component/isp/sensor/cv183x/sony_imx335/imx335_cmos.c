...
#define SENSOR_IMX335_2M_WIDTH	1920
#define SENSOR_IMX335_2M_HEIGHT 1080
...
#define IMX335_RES_IS_2M(w, h)      ((w) == SENSOR_IMX335_2M_WIDTH && (h) == SENSOR_IMX335_2M_HEIGHT)
...
static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (pstSnsState->u8ImgMode == IMX335_MODE_5M60 ||
	    pstSnsState->u8ImgMode == IMX335_MODE_4M60 ||
	    pstSnsState->u8ImgMode == IMX335_MODE_2M60)
		fps = 60;
#if 0
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case IMX335_MODE_5M60:
	case IMX335_MODE_4M60:
	case IMX335_MODE_2M60:
		if ((f32Fps <= f32MaxFps) && (f32Fps >= f32MinFps)) {
			u32VMAX = u32Vts * f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
		} else {
			CVI_TRACE_SNS(CVI_DBG_ERR, "Unsupport Fps: %f\n", f32Fps);
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			...
			else if (IMX335_RES_IS_2M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				/* sample code would set image mode then set wdr mode.
				 * In this sensor 2MP30 linear is not implemented and driver returns as failure.
				 * To prevent that, we force it to 2MP30 WDR mode.
				 */
				u8SensorImageMode = IMX335_MODE_2M30_WDR;
			} else {
		...
		if (IMX335_RES_IS_5M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
			u8SensorImageMode = IMX335_MODE_5M60;
		else if (IMX335_RES_IS_4M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
			u8SensorImageMode = IMX335_MODE_4M60;
		else if (IMX335_RES_IS_2M(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height))
			u8SensorImageMode = IMX335_MODE_2M60;
		else {
			CVI_TRACE_SNS(CVI_DBG_ERR, "Not support! Width:%d, Height:%d, Fps:%f, WDRMode:%d\n",
			       pstSensorImageMode->u16Width,
			       pstSensorImageMode->u16Height,
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	if (pstSnsState->enWDRMode != WDR_MODE_NONE ||
	    pstSnsState->u8ImgMode == IMX335_MODE_5M60 ||
	    pstSnsState->u8ImgMode == IMX335_MODE_4M60 ||
	    pstSnsState->u8ImgMode == IMX335_MODE_2M60) {
		pstRxAttr->mac_clk = RX_MAC_CLK_500M;
		pstRxAttr->mipi_attr.raw_data_type = RAW_DATA_10BIT;
		if (pstSnsState->enWDRMode == WDR_MODE_NONE)

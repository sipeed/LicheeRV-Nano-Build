 ...
 * global variables                                                            *
 ****************************************************************************/
static uint32_t *gain_table = imx327_gain_table;
static const uint32_t g_u32AEResponseFrame = 3; //MAX=N+1
...
#define IMX327_FULL_LINES_MAX_2TO1_HDR  (0x3FFFF)    // considering the YOUT_SIZE and bad frame
...
static CVI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case WDR_MODE_2To1_LINE:
		if (pstSnsState->u8ImgMode == IMX327_MODE_720P30_HDR) {
			pstAeSnsDft->f32Fps = g_astImx327_mode[IMX327_MODE_720P30_HDR].f32MaxFps;
			pstAeSnsDft->f32MinFps = g_astImx327_mode[IMX327_MODE_720P30_HDR].f32MinFps;
 		} else {
			pstAeSnsDft->f32Fps = g_astImx327_mode[IMX327_MODE_1080P30_HDR].f32MaxFps;
			pstAeSnsDft->f32MinFps = g_astImx327_mode[IMX327_MODE_1080P30_HDR].f32MinFps;
		}
		...
		pstAeSnsDft->u32AEResponseFrame = g_u32AEResponseFrame;
	...
	default:
		...
		} else if (pstSnsState->u8ImgMode == IMX327_MODE_720P30) {
			pstAeSnsDft->f32Fps = g_astImx327_mode[IMX327_MODE_720P30].f32MaxFps;
			pstAeSnsDft->f32MinFps = g_astImx327_mode[IMX327_MODE_720P30].f32MinFps;
 		} else {
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case IMX327_MODE_1080P30_HDR:
	case IMX327_MODE_720P30_HDR:
		...
		if (u32VMAX > IMX327_FULL_LINES_MAX_2TO1_HDR)
			u32VMAX = IMX327_FULL_LINES_MAX_2TO1_HDR;
		break;

	case IMX327_MODE_1080P30:
	case IMX327_MODE_1080P60:
	case IMX327_MODE_720P30:
		if ((f32Fps <= f32MaxFps) && (f32Fps >= f32MinFps)) {
			u32VMAX = u32Vts * f32MaxFps / DIV_0_TO_1_FLOAT(f32Fps);
		} else {
			CVI_TRACE_SNS(CVI_DBG_ERR, "Unsupport Fps: %f\n", f32Fps);
		...
		if (u32VMAX > IMX327_FULL_LINES_MAX)
			u32VMAX = IMX327_FULL_LINES_MAX;
		break;
	default:
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	...
	if (pstSnsState->enWDRMode != WDR_MODE_NONE) {
		pstIspCfg->frm_num = 2;
		memcpy(&pstIspCfg->img_size[0], &pstMode->astImg[0], sizeof(ISP_HDR_SIZE_S));
		memcpy(&pstIspCfg->img_size[1], &pstMode->astImg[1], sizeof(ISP_HDR_SIZE_S));
	} else {
		pstIspCfg->frm_num = 1;
		memcpy(&pstIspCfg->img_size[0], &pstMode->astImg[0], sizeof(ISP_HDR_SIZE_S));
	}
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		if (pstSnsState->u8ImgMode == IMX327_MODE_1080P30_HDR)
	...
	case WDR_MODE_2To1_LINE:
		if (pstSnsState->u8ImgMode == IMX327_MODE_1080P30)
			pstSnsState->u8ImgMode = IMX327_MODE_1080P30_HDR;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u32AddrByteNum = IMX327_ADDR_LEN;
			pstI2c_data[i].u32DataByteNum = IMX327_DATA_LEN;
		}
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (SENSOR_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = IMX327_MODE_720P30;
			} else if (SENSOR_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
			...
			if (SENSOR_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = IMX327_MODE_720P30_HDR;
			} else if (SENSOR_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = IMX327_MODE_1080P30_HDR;
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	if (pstRxAttr->input_mode == INPUT_MODE_MIPI)
		memcpy(pstRxAttr, &imx327_mipi_attr, sizeof(*pstRxAttr));
	else
		memcpy(pstRxAttr, &imx327_sublvds_attr, sizeof(*pstRxAttr));

	pstRxAttr->img_size.x = g_astImx327_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.y = g_astImx327_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		pstRxAttr->mipi_attr.hdr_mode = CVI_MIPI_HDR_MODE_NONE;
	}
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &imx327_mipi_attr;
	int i;
...
static CVI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_exit = sensor_i2c_exit;
	...
	pstSensorExpFunc->pfn_cmos_set_sns_reg_info = cmos_set_sns_regs_info;
	return CVI_SUCCESS;
}

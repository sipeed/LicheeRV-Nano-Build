...
#include "rtl8733bs.h"		/* rtl8733bs_get_interrupt(), rtl8733bs_clear_interrupt() and etc. */
 ...
 *	_FAIL(0)	Fail
 */
u32 rtl8733bs_read_port(struct dvobj_priv *d, u32 cnt, u8 *mem)
{
...
static u32 sdio_read_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	return rtl8733bs_read_port(pintfhdl->pintf_dev, cnt, recvbuf->pbuf);
}
 ...
 *	_FAIL(0)	Fail
 */
u32 rtl8733bs_write_port(struct dvobj_priv *d, u32 cnt, u8 *mem)
{
	u32 txaddr, txsize;
	u32 ret = _FAIL;
...
static u32 sdio_write_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	ret = rtl8733bs_write_port(d, cnt, xmitbuf->pdata);
...
static void sd_recv_loopback(PADAPTER padapter, u32 size)
{
	...
	ret = rtl8733bs_read_port(d, readsize, preadbuf);
	...
	if (ploopback)
		...
		u32 i;
		RTW_INFO("%s ploopback is null, free preadbuf\n", __func__);
		RTW_INFO("%s: drop pkt\n", __func__);
...
static struct recv_buf *sd_recv_rxfifo(PADAPTER adapter, u32 size)
{
	...
	/* 2. alloc skb */
	pkt = rtl8733bs_alloc_recvbuf_skb(recvbuf, bufsz);
	if (!pkt) {
	...
	ret = rtl8733bs_read_port(d, bufsz, rbuf);
	if (_FAIL == ret) {
		RTW_ERR("%s: read port FAIL!\n", __FUNCTION__);
...
void sd_int_dpc(PADAPTER adapter)
{
	...
	if (phal->sdio_hisr & BIT_SDIO_AVAL_8733B)
		_rtw_up_sema(&adapter->xmitpriv.xmit_sema);

#endif /* CONFIG_SDIO_TX_ENABLE_AVAL_INT */

	if (phal->sdio_hisr & BIT_SDIO_CPWM1_8733B) {
		...
		report.state = rtw_read8(adapter, REG_SDIO_HCPWM1_V2_8733B);
	...
	if (phal->sdio_hisr & BIT_SDIO_TXERR_8733B) {
		u32 status;
		u32 addr;

		addr = REG_TXDMA_STATUS_8733B;
	...
	if (phal->sdio_hisr & BIT_SDIO_TXBCNOK_8733B)
		RTW_INFO("%s: SDIO_HISR_TXBCNOK\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_TXBCNERR_8733B)
		RTW_INFO("%s: SDIO_HISR_TXBCNERR\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_RXFOVW_8733B)
		RTW_INFO("%s: Rx Overflow\n", __FUNCTION__);

	if (phal->sdio_hisr & BIT_SDIO_RXERR_8733B)
		RTW_INFO("%s: Rx Error\n", __FUNCTION__);
#ifdef CONFIG_SDIO_MULTI_FUNCTION_COEX
#if DBG_SDIO_MULTI_FUNCTION_COEX
	if (phal->sdio_hisr & BIT_BT_INT_8733B)
		RTW_INFO("%s: SDIO bus resume to available\n", __FUNCTION__);
#endif
#endif

	if (phal->sdio_hisr & BIT_RX_REQUEST_8733B) {
		...
		if (!ex_hal_sdio_multi_if_bus_available(adapter)
			&& !(phal->sdio_hisr & BIT_BT_INT_8733B)
		) {
			#if DBG_SDIO_MULTI_FUNCTION_COEX
		...
		phal->sdio_hisr ^= BIT_RX_REQUEST_8733B;
		...
		do {
			...
			#ifdef CONFIG_MAC_LOOPBACK_DRIVER
			sd_recv_loopback(adapter, rx_len);
			#else
			precvbuf = sd_recv_rxfifo(adapter, rx_len);
			if (precvbuf) {
				rtl8733bs_rxhandler(adapter, precvbuf);
			...
			#endif /* CONFIG_MAC_LOOPBACK_DRIVER */
			rx_len = 0;
			rtl8733bs_get_interrupt(adapter, NULL, &rx_len);
		} while (1);
...
void sd_int_hdl(PADAPTER adapter)
{
	...
	rtl8733bs_get_interrupt(adapter, &phal->sdio_hisr, &phal->SdioRxFIFOSize);
	if (phal->sdio_hisr & phal->sdio_himr) {
		phal->sdio_hisr &= phal->sdio_himr;
		sd_int_dpc(adapter);
		rtl8733bs_clear_interrupt(adapter, phal->sdio_hisr);
	} else {
...
{
	rtl8733bs_disable_interrupt_but_cpwm2(adapter);
	return _SUCCESS;
}
...
u8 RecvOnePkt(PADAPTER adapter)
{
	...
	rtl8733bs_get_interrupt(adapter, &phal->sdio_hisr,
				&phal->SdioRxFIFOSize);
